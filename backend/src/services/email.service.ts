...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,sans-serif;">
  ...
  async sendResetPasswordEmail(params: {
    ...
    if (!transporter) { logger.warn('[EMAIL] SMTP non configuré — reset password non envoyé'); return; }
    try {
      await transporter.sendMail({
        from: process.env.SMTP_FROM || `"SenLocaDesk" <${process.env.SMTP_USER}>`,
        to: params.to,
        subject: `Réinitialisation de votre mot de passe — SenLocaDesk`,
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:40px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        <!-- Header -->
        <tr><td style="background:#E53935;padding:32px 40px;text-align:center;">
          <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.8);letter-spacing:2px;text-transform:uppercase;font-weight:bold;">&lt;/&gt; INNOSOFT CREATION</p>
          <h1 style="margin:12px 0 4px;font-size:26px;font-weight:bold;color:#ffffff;">Réinitialisation du mot de passe</h1>
          <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.85);">SenLocaDesk — Plateforme de location de véhicules</p>
        </td></tr>
        <!-- Body -->
        <tr><td style="padding:40px;">
          <p style="margin:0 0 16px;font-size:15px;color:#333;">Bonjour <strong>${params.prenom} ${params.nom}</strong>,</p>
          <p style="margin:0 0 24px;font-size:14px;color:#555;line-height:1.6;">
            Vous avez demandé la réinitialisation de votre mot de passe SenLocaDesk.<br>
            Cliquez sur le bouton ci-dessous pour choisir un nouveau mot de passe.
          </p>
          <!-- CTA -->
          <table cellpadding="0" cellspacing="0" style="margin:0 auto 24px;">
            <tr><td style="background:#E53935;border-radius:8px;text-align:center;">
              <a href="${params.resetUrl}" style="display:inline-block;padding:14px 36px;color:#ffffff;font-size:15px;font-weight:bold;text-decoration:none;">
                🔑 Réinitialiser mon mot de passe
              </a>
            </td></tr>
          </table>
          <!-- Avertissement -->
          <table width="100%" cellpadding="12" cellspacing="0" style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;margin-bottom:24px;">
            <tr><td style="font-size:13px;color:#f57f17;">
              ⚠️ Ce lien est valable <strong>1 heure</strong>. Si vous n'avez pas fait cette demande, ignorez cet email — votre mot de passe reste inchangé.
            </td></tr>
          </table>
          <p style="margin:0;font-size:12px;color:#999;">Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur :<br>
            <a href="${params.resetUrl}" style="color:#E53935;word-break:break-all;">${params.resetUrl}</a>
          </p>
        </td></tr>
        <!-- Footer -->
        <tr><td style="background:#1a1a1a;padding:20px 40px;text-align:center;">
          <p style="margin:0;font-size:12px;color:#888;">&lt;/&gt; <strong style="color:#fff;">Innosoft Creation</strong> — <a href="https://innosft.com" style="color:#E53935;text-decoration:none;">innosft.com</a></p>
          <p style="margin:4px 0 0;font-size:11px;color:#666;">Cet email a été généré automatiquement. Merci de ne pas y répondre directement.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
