...
import {
  ...
  AlertTriangle, ShieldCheck, Plus, Trash2, FileText, X, Mail,
} from 'lucide-react';
...
export function TenantDetailPage() {
  ...
  const [editEmailUser, setEditEmailUser] = useState<TenantUser | null>(null);
  ...
  async function handleUpdateEmail() {
    if (!editEmailUser || !tenant) return;
    if (!newEmail || !newEmail.includes('@')) { setEmailError('Email invalide'); return; }
    setSavingEmail(true); setEmailError('');
    try {
      await api.patch(`/tenants/${tenant.id}/users/${editEmailUser.id}/email`, { email: newEmail });
      setEditEmailUser(null);
      window.location.reload();
    } catch (err: unknown) {
      setEmailError((err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Erreur lors de la mise à jour');
    } finally {
  ...
  return (
    <div className="space-y-6">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {tenant.users.map(u => (
                      <tr key={u.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <button
                            onClick={() => { setEditEmailUser(u); setNewEmail(u.email); setEmailError(''); }}
                            className="flex items-center gap-1 text-xs text-gray-400 hover:text-asm-vert transition-colors"
      ...
      {/* Modal: Modifier email utilisateur */}
      {editEmailUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-start justify-between">
              <h2 className="text-base font-bold text-gray-900 flex items-center gap-2">
                <Mail className="h-5 w-5 text-asm-vert" /> Modifier l'email
              </h2>
              <button onClick={() => setEditEmailUser(null)} className="text-gray-400 hover:text-gray-600" aria-label="Fermer">
                <X className="h-5 w-5" />
              </button>
            </div>
            <p className="text-sm text-gray-500">
              Utilisateur : <span className="font-semibold text-gray-800">{editEmailUser.prenom} {editEmailUser.nom}</span>
              <span className={`ml-2 inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${ROLE_COLORS[editEmailUser.role] ?? 'bg-gray-100 text-gray-600'}`}>{editEmailUser.role}</span>
            </p>
            <div>
            ...
            {emailError && <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{emailError}</p>}
            <div className="flex gap-3">
              <button onClick={() => setEditEmailUser(null)} className="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50">Annuler</button>
              <button onClick={handleUpdateEmail} disabled={savingEmail || newEmail === editEmailUser.email} className="flex-1 px-4 py-2.5 rounded-lg bg-asm-vert text-white text-sm font-semibold hover:bg-asm-vert/90 disabled:opacity-50">
